...
export class OtpCountdownService implements OnDestroy {
  private readonly EXPIRES_AT_KEY = OtpStorageKeys.OTP_EXPIRES_AT;
  private readonly TIMER_KEY = OtpStorageKeys.TIMER;
  ...
  private countdownState$ = new BehaviorSubject<CountdownState>({
    remainingSeconds: 0,
    isExpired: true,
    canResend: true,
    sentAt: null,
   ...
   * 获取倒计时状态的 Observable
   */
  getState(): Observable<CountdownState> {
  ...
  restoreFromCache(): boolean {
    // 优先从绝对到期时间恢复
    const cachedExpiresAt = this.getNumberFromStorage(this.EXPIRES_AT_KEY);
    
    if (cachedExpiresAt && cachedExpiresAt > Date.now()) {
      // 缓存的到期时间仍然有效
      const expiredAt = new Date(cachedExpiresAt);
      this.targetTime = expiredAt;
      
      // 恢复发送时间
      const cachedSentAt = this.getNumberFromStorage(this.SENT_AT_KEY);
      if (cachedSentAt) {
        this.sentAt = new Date(cachedSentAt);
      }
    ...
    // 尝试从旧的 Timer 键恢复（降级方案）
    const cachedTimer = this.getNumberFromStorage(this.TIMER_KEY);
    if (cachedTimer && cachedTimer > 0) {
      this.start(cachedTimer);
      return true;
    }

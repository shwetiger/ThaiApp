...
export class DefaultOptSettingComponent implements OnInit {
  ...
  onSubmit(){
    ...
    if(this.formPage=='otppage' || this.formPage=='withdrawaladd')
    {
  ...
  SaveOtptype() { 
   ...
   SaveOtptypeandgetotp() { 
    this.token = this.storage.retrieve('token');    
    const headers = new HttpHeaders();
     this.http.post(this.funct.ipaddress + 'user/setusersmstypeAndGetOTP?type='+this.selectedType +'&phone_no='+this.phoneNumber,{ headers:headers })
     .pipe(
       catchError(this.handleErrorMessage.handleError.bind(this,this.formPage))
      )
     .subscribe(
    ...
    this.spinner.show("submitLoading");
     ...
     .subscribe(
       result => {
         ...
         if(this.gmailResponse.message==" Only one text message can be sent within 60 seconds ")
         {
          this.toastr.error("", this.translateService.instant("otp_wait_alert"), {
            timeOut: 3000,
            positionClass: 'toast-top-center',
            });
            this.common.submitLoading= false;         

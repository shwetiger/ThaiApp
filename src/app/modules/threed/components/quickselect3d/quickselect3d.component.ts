...
export class Quickselect3dComponent implements OnInit {
  ...
  cannotSmallThan: any;
  amount: any;
  //selectedNumberList:Array<{betNumberValue:string}>=[];
  ...
  assignTrippleNubmers(){
    //this.trippleNumbers.push({betNumberValue:000,isUse:1});
    for(let i =0;i<=9;i++){
  ...
  selectTrippleNubmers(objNum):{ betNumberValue: string; isUse: number; }[]{
    //  var source = this.betNumberList.filter(f=>f.betnumber == objNum.betnumber);
  ...
  selectHundredNubmers(objNum):{ betNumberValue: string; isUse: number; }[]{
    ...
    {      
      if(isUse == 1){  
       ...
       // this.selectedRangeNumberList.splice(this.selectedRangeNumberList.findIndex(x=>x.betNumberValue === objNum.betNumberVaule),1);
        this.count -=100;

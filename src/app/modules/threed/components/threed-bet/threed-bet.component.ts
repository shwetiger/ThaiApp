...
export class ThreedBetComponent implements OnInit {
  ...
  threeDBet(){  
    ...
    //var source = this.betNumberList.filter(f=>f.isUse == 1);//here filter only isUse =1 , in source not included isUse =0, old code
    let source = this.betNumberList.filter(f=>f.isUse == 1);
    if(source.length > 0)//old code
    {
      if(this.checkBetAmount())
      {
        var c=0; /*XXXX here check MH*/

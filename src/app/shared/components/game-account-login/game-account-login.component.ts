    ...
    async getGameUserBalance()
    {
      ...
      for(let i=0;i< this.gameProviderId.length;i++){
        ...
        if(this.gameProviderId==14)   
        {
          ...
          .then((res) => {
            this.isUserLoggedIn= true;
            this.gameUserBalance.push({ providerId: this.gameProviderId[i],balance: parseInt(res.data.data.balance),dataUrl: res.data.data.dataUrl,name: res.data.data.display_name});
            this.common.closeLoadingSubmit= false;
            this.spinner.hide('closeLoadingSubmit');
